...
def extract_embedding(out):
    ...
    _, embeddings = out  # распаковываем кортеж, игнорируя первый элемент
    return embeddings
...
# emb_model будет загружаться через функцию load_embedding_model (чтобы можно было переключать arcface/CE)
emb_model = None
